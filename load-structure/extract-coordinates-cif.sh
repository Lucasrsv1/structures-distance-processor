...
x_col=$((`head -n $x_header_line "$decompressed_file" | tail -n 25 | tac | awk '$1 ~ /^loop_/ { print NR }' | head -n 1` - 1))

y_col=$(($x_col + 1))
z_col=$(($x_col + 2))

model_col=$((`head -n $model_header_line "$decompressed_file" | tail -n 25 | tac | awk '$1 ~ /^loop_/ { print NR }' | head -n 1` - 1))
alt_pos_col=$((`head -n $alt_pos_header_line "$decompressed_file" | tail -n 25 | tac | awk '$1 ~ /^loop_/ { print NR }' | head -n 1` - 1))
